...
The Phase 3A signed/absolute log-offset candidates are rejected as primary due to their zero-floor and pseudocount dependence. The provisional primary is the four-category supported-detection state in `docs/zero_aware_endpoint_design.md`. It was chosen from prespecified thresholds using only replicate and quantifier behavior. At 1 TPM with 2/2 Illumina and 3/4 ONT support, 13,375 of 49,309 independently expressed transcripts are indeterminate, not silently removed as zeros. Pairwise detection agreement is 0.869 for Illumina and 0.880 for ONT; however, only 30,445 rows are complete for four-replicate NanoCount, and its status agreement with Salmon is 0.519 on that subset. Thus the endpoint remains a **candidate, not a frozen primary**.

Conditional rank difference among BOTH is secondary. Strictly-positive log ratio, isoform-fraction difference and transcript completeness remain exploratory. Spike-in relative true error remains separate and not yet calibrated to all libraries. No structure value or structure-versus-outcome association was used in this selection. The historical text below records earlier candidate decisions and does not override the Phase 3B status.
