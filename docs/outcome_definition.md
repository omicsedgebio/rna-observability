...
# Phase 3B superseding zero-aware proposal

The earlier offset-based signed log abundance formula in this document is retained as Phase 2/3A history and is **not** the current primary candidate. Phase 3B proposes a two-part endpoint without adding arbitrary mass to zeros. See `docs/zero_aware_endpoint_design.md` for exact runs, common-universe closure, threshold grid and measurement-only QC.

The primary candidate is a four-category detection state under Salmon-family SG-NEx K562 estimates: a transcript is detected in Illumina when both held-out Illumina preparations have reclosed TPM at least 1, and in ONT direct RNA when at least three of four independent preparations do. Zero qualifying libraries means absent on that platform; intermediate counts are indeterminate. Determined states form BOTH, ILLUMINA_ONLY, DIRECT_RNA_ONLY and NEITHER. Independent Illumina Rep3 TPM at least 1 defines abundance eligibility. These categories are quantifier/protocol phenotypes, not biological absence or true error.

Among BOTH, the quantitative secondary is median ONT within-library percentile rank minus median Illumina within-library percentile rank over a fixed Ensembl 91 universe. A positive-only log2 ratio is exploratory only when all included estimates are strictly positive. It has no pseudocount. The candidate endpoint is **not frozen** because ONT quantifier sensitivity and indeterminate-call burden remain substantial.
